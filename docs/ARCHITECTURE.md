...
Implemented: the three probe tools, the shared headers, the build and CI.

Not implemented: `t150d`, `t150-dinput8.dll`, `t150boot`, `t150ctl`. The
build order is Phase 0 first (see [PROBES.md](PROBES.md)), because a single
measurement decides whether any of the rest is worth writing: whether an
unprivileged `IOHIDDeviceSetReport` physically moves the wheel.
